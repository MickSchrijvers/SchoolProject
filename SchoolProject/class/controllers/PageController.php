...
class PageController extends PageModel {
	...
	private function set_pageObj()
	{
		...
		if (file_exists('class\pages\\' . $objName . '.php')) {
	...
	{	
		$filename = 'views\parts\\' . $file . '.phtml';
	...
	{
		$pagename = (isset($this->pagename) ? $this->pagename : 'home');
		$filename = 'views\templates\\' . $pagename . '.phtml';

		if ( file_exists($filename) ) {
			include $filename;
		} else if (!$this->pageContent) {
			include 'views\templates\404.phtml';
		} else {
			include 'views\templates\default.phtml';
		}
	}
}